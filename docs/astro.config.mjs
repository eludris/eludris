...
const remarkGenerateSearchIndex = () => {
  return (tree, file) => {
    ...
    if (file.history[0]) {
      ...
      const entry = AUTODOC_ENTRIES.find(e => sections[sections.length - 2].text == e.split('/')[1].split('.')[0].replace(/(?:^|_)([a-z0-9])/gm, (_, p1) => p1.toUpperCase()).replace(/[A-Z]/gm, ' $&').trim()).split('.')[0];
    ...
    sections = sections.filter(s => s.text).map(s => ({
      route: `${baseRoute}${s.id ? '#' + s.id : ''} `,
      ...s
    }));
...
export default defineConfig({
  ...
  markdown: {
    ...
    rehypePlugins: [rehypeAccessibleEmojis, rehypeSlug, [rehypeAutolinkHeadings, {
      behavior: 'before',
      content() {
